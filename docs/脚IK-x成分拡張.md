> **2026-09-14 追記**: 以下は文書の座標系 Σ_S（x 右 / y 前）で、膝軸と足首ロール軸が同じ軸
> （足首は上側がロール・下側がピッチ）だった頃の導出 (X-n)。実機は足首の上側ピボットがピッチ・
> 下側がロールだったため、`leg_kinematics.hpp` は機体座標 Σ_B（x 前 / y 左 / z 上）で
> 膝と足首ピッチが同軸の導出 (B-n) に書き直した。**現行の導出はそのヘッダの冒頭を読むこと。**
> この文書は経緯として残す。
