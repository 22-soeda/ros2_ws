...
**足の位置はそのままで骨盤の横振りだけが増える**（狭く取れば減る。今の動歩行は計画 155mm・
実機 ±89.3mm。上の「横の寄り」の節）。経緯（2026-09-19 に `ds_time` 0.3 を入れ、両足支持の間に骨盤が支持足へ寄るようになったので
計画だけ広げる工夫はやめた。2026-09-20 に足を股の真下へ）。★`foot.y` は上の節のとおり
**モードごと**（動歩行は `walk_mode: dynamic: foot: y`）。理由と候補の表は `gait.yaml` の
`foot_spacing` のコメント。
