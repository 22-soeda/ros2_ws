...
脱力から復帰する唯一の手段でもある。誤って触ってもトルクが入らないよう長押しにしてある。
...
| 右スティックを左へ | `axes[2] = +0.88` | `angular.z = +0.400` |
...
* `/autonomy` は teleop 側で決めた新設トピック。behavior を書くときに ros-architecture §4
  のトピック表へ追記すること。
* 自律動作中に teleop が黙るので、behavior が上がっていないまま自律に入ると `/cmd_walk`
  が完全に止まる。motion の「途切れたら止まる」が効いて機体は停止する (安全側) が、
  操作者から見ると「入ったのに動かない」になる。behavior ができたら状態を `/motion/state`
  か ui に出して分かるようにしたい。
