...
2. **間合いの中での転倒判定を凍結しない**（`tracking.py` の `_step_close`）。
   ...
   **間合いに入ってから相手が倒れた場合を一度も検出できない**。技を当てて倒した
   ...
   2026-09-20 に 2 つ足した（`docs/相手機の認識.md` §5）。**「平たい」の証拠**
   （上端 < 0.22 m かつ 幅 > 1.3 × 上端。較正が要らず、平たい形は H_o の較正にも入れない。
   再開時に倒れた相手の前へ置かれると倒れた高さを立位高さとして覚えてしまうため）と、
   **「横に広い」は立位の高さに達していないときだけ数える**（腕を広げて立つ相手を
   転倒にしない。以前は幅だけで転倒側に数えていた）。
