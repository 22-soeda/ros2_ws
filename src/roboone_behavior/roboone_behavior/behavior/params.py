...
class TuneParams:
    ...
    # --- 転倒判定 (§2.3) --------------------------------------------------
    # **高さのしきい値はここに無い** (2026-09-23)。相手が転倒しているかどうかの
    # 1 フレームの判断は検出器がクラスタ上端 z_top の絶対値で行い、行動層は
    # Opponent.kind を受けるだけ。境界を動かすのは opponent_detector.yaml の
    # match.fallen_top_max / robot_top_max。ここに残るのは時間の門だけ。
    #: [s] 転倒・復帰の継続時間 T_down / T_up。1 フレームの分類のばたつきで
    #: 状態が暴れないための門。長くすると反応が遅れ、短くすると誤検出が通る
    ...
    #: [m] 間合いの中で転倒と認めるのに要る横幅。上端が視野で切れても
    #: クラスタが横に広がることはない、という切り分けに使う。既定は以前の
    #: 「H_o の 0.75 倍」を H_o の初期値 0.35 m で置き換えた値 (0.26)
    fallen_width_min: float = 0.26
