...
class TuneParams:
    ...
    #: 較正の要らない「平たい」の証拠 (docs/相手機の認識.md §5。2026-09-20)。
    #: 二足歩行機は立位で 上端 > 幅、横倒しで 幅 > 上端 に反転する。上端がこの高さ [m]
    #: より低く、かつ 幅 > flat_aspect × 上端 なら転倒側の証拠にする。再開時に近距離へ
    #: 置かれて H_o の較正が取れないときの保険。上端の上限を併せて見るのは、腕を広げて
    #: 立つ相手 (幅 0.7 m・上端 0.45 m) を平たいと言わないため
    flat_top_max: float = 0.22
    flat_aspect: float = 1.3
