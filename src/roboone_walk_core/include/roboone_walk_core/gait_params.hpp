...
{
  // --- 力学 -----------------------------------------------------------
  double z_c = 0.16;               // [m]   骨盤 (≒重心) 高さ。ω = sqrt(g/z_c)
  double gravity = 9.81;           // [m/s^2]
  double t_step = 0.40;            // [s]   1 歩の周期 T
  double foot_spacing = 0.08;      // [m]   左右の足間隔 W
  ...
  // を超えると、吸収残りが e^{ωT} ≈ 22.9 倍に増幅されて発散する (params.py 参照)。
  double a_max[2] = {0.15, 0.08};  // [m/s^2] (x, y) のレート制限
