...
### `viz/serve_imu3d.py` — 姿勢を EKF に通して 3D で見る

**ロール・ピッチ・ヨーと実際の傾きの対応付けが合っているかを目で確かめる**ための道具。
`/camera/imu` を購読して姿勢を推定し、ブラウザに 3D で出す。推定結果は publish しない
（本番の姿勢推定は `docs/ros-architecture.md` どおり imu_filter が `/imu/data` に出す担当）。
...
python3 src/realsense_bringup/viz/serve_imu3d.py          # 既定 8104 番
python3 src/realsense_bringup/viz/serve_imu3d.py --demo   # 実機なしで表示だけ確かめる
```

bind 先は既定で **ssh 元から見えるこの機体の IP**（`SSH_CONNECTION` から取る）。
起動時に URL を印字するのでそれを開く。ssh 経由でなければ `0.0.0.0`。
`--bind` / `--port` / `--topic` で変えられる。

画面に出るもの:

- 太い矢印 = 機体の x(前) / y(左) / z(上) の**正方向**、地面の細い矢印 = 世界の同じ 3 軸。
  カメラの箱と、前面（+x）のレンズも描くので、どちらを向いているかが分かる。
- ロール・ピッチ・ヨー（Z-Y-X）と、その時系列グラフ。
- **加速度だけから出した傾き**を併記する。静止していれば EKF と一致するはずで、
  ずれていれば軸の載せ替えか式のどちらかが間違っている、という検算になる。
- 光学フレームの生値と Σ_B に直した値の両方。

対応付けの確認は、カメラを手で持って次のとおり動かす。

| 動かし方 | 期待 |
|---|---|
| 右を下げる | roll が + |
| お辞儀させる（レンズを下へ） | pitch が + |
| 左へ回す | yaw が + |

#### 座標系

`/camera/imu` の frame_id は `camera_imu_optical_frame`（**X=右 / Y=下 / Z=前**）。
この機体の Σ_B は REP-103（**x=前 / y=左 / z=上**）なので、載せ替えは

    x_b = +z_o     y_b = -x_o     z_b = -y_o

静止時の加速度計は「上向きに +g」を測る。水平・正立なら Σ_B で `(0, 0, +9.81)`、
光学で `(0, -9.81, 0)`。README 冒頭の「正立・水平なら Y(下) に約 -9.8」と同じ話。

#### ヨーは 6 軸 IMU では観測できない

加速度計は重力しか見ないので、**重力の軸まわりの回転（＝ヨー）は補正できない**。
画面のヨーはジャイロの積分で、ゆっくりドリフトする。押さえるには地磁気を足すか、
歩行なら足裏の接地拘束を使うしかない。ロール・ピッチとは信頼度が違うので、
画面でも σ を分けて出してある（ヨーの欄は鉛直まわりの σ）。

ついでに、鉛直まわりの**ジャイロバイアスも観測できない**。上限（±0.05 rad/s、
`rs_imu_test` の実測 0.005 rad/s の 10 倍）を付けていないと、手で振り回した回転を
バイアスが吸ってしまい、置いた後もヨーが流れ続ける（実機で 0.32 rad/s まで育つのを確認した）。

ピッチ ±90° 付近ではロールとヨーが縮退する（ジンバルロック）。カメラを机に伏せて置くと
ちょうどここに入るので、数字が暴れるのは式の性質であって異常ではない。
