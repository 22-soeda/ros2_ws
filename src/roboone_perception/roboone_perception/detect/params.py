...
class TuneParams:
    ...
    #: 垂直画角の下端 (俯角 29 度) より手前の床が写らないので、高さ 0.40 m なら 0.72 m
    ...
    #: 影をまたいで足す床の成分の最小セル数。物の垂直な面が床の帯に落とす数セルを
    #: 床と取り違えないため (pipeline._bridge_shadows)
    bridge_min_cells: int = 6
    ...
    #: リングの縁から外へこのセル数の帯の中で「外の物」を探す。10 セル = 50 cm
    #: (規則: 操縦者はリングから 50 cm 以上離れる。腕から胴体までたどれる幅)
    intrude_band_cells: int = 10
    #: 内側の塊からつながる「外の物」がこのセル数以上なら、外から差し込んでいる
    #: (レフリーの腕など) として相手にしない。0 で無効
    intrude_min_cells: int = 4
    #: 「外の物」とみなすセルの最小点数。深度の段差に出る飛び画素を数えないため
    intrude_cell_points: int = 3
