...
//   * 出す量は rate [mm/s] で頭を抑える（ds_time = 0 の動歩行では割合が歩の境界で飛ぶため）。
//     **ただし ds_time > 0 なら rate >= sink / ds_time が要る**（下の LoadFfParams::rate）。
...
struct LoadFfParams
{
  ...
  //! **0 = 補正なし（既定）。** 2026-09-19 まで 8.0 が残っていて、
  //! motion_node.yaml を通さずに使うと立位でも両脚が 4mm (= sink/2) 伸びていた
  //! （HOLD でも tickWalk が回り、両足接地の割合は 0.5 ずつになるため）
  double sink = 0.0;
  //! [mm] 1 脚あたりの伸ばし量の上限（測り違い・α の暴れに対する保険）
  double clamp = 10.0;
  //! [mm/s] 伸ばし量の変化の速さの上限（下の注記）。
  //! ★**ds_time > 0 なら rate >= sink / ds_time にすること**（2026-09-22）。両足支持の間に
  //! 割合が 0<->1 を線形に動くので、追い切れないと位相が遅れ、(a) 荷重を受け取る支持脚が
  //! 伸び切らないまま沈む (b) 離地の瞬間に前の支持脚へ残りが出て遊脚が床を押す、の 2 つが
  //! **どちらも体を支持脚の外側へ押す**。sink 8 / ds_time 0.2 / t_step 0.45 で rate 10 だと
  //! 1 歩 (0.65s) で 6.5mm しか動けず、定常で 0<->6.5mm の三角波になり離地で 4.5mm 残る。
  double rate = 10.0;
...
///
/// なので目標へは rate [mm/s] で近づける。
///
/// ★**追い付かないことは害ではない、とは言えない**（2026-09-22。下は計算で、実機未確認）。ds_time > 0
///   なら割合は飛ばずに Td 秒で連続に動くので、rate が sink/ds_time より小さいと前送りが
///   薄まるのではなく**位相が遅れる**。遅れた分は (a) 新しい支持脚が伸び切らないまま荷重を
///   受けて沈む (b) 離地の瞬間に前の支持脚へ伸ばし量が残り、遊脚が床を押す という形で出て、
///   どちらも体を**支持脚の外側**へ押す（外への倒れは次の一歩で受けられない）。実機では
///   sink 8.0 / rate 10 の動歩行で「蹴る力が大きく支持脚側へ倒れる」と出たが、bag は無い。
