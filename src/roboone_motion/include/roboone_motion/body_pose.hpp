...
// 両足裏を平面ごと回す（板）
// ===========================================================================
// 安定化の補正のもう 1 つの掛け方（stabilizer.hpp「足裏の補正の 2 つの方式」）。
// 足首だけを回すと、左右の足裏が別々の平面に乗る。足間隔 140mm あるので、
// ロールを 4 deg 入れるだけで左右の足裏の高さが 10mm 食い違う。両足が床に
// 着いていれば脚どうしが突っ張り合い、片足が浮いていればその足が床へ押し込まれる
// （2026-09-18 の bag で、遊脚が床の下 10mm を狙っていた原因がこれ）。
//
// 板では、**両足裏を 1 枚の板と見て、板ごと回す**。足裏の向きだけでなく位置も
// 一緒に回るので、2 足の相対位置と相対姿勢は回す前と同じまま保たれる。
// サーフボードに乗った足の関係が変わらないのと同じで、代わりに脚が伸び縮みする
// （中点から離れているほど大きい。ロール u なら左右とも (W/2)·sin u）。
//
//     p' = c + Q·(p − c)      R' = Q·R      Q = Ry(pitch)·Rx(roll)
//
// 回転の中心 c は 2 足の中点。高さは**低いほうの足裏**（= 床）に取る。中点を軸に
// すると伸び縮みが左右で同じだけになり、要る脚の長さがいちばん小さくなる。
//
// ★足首だけ回す方式との違いは「足裏の位置も動くか」だけで、支持足から見た胴体の
//   傾き（＝立て直しの効き目）は同じ。docs/commands.md「安定化」を見よ。

/// 両足裏を、同じ平面に乗せたまま (roll, pitch) [rad] だけ回す。Σ_U で掛ける。
///
/// roll + で左の縁が上がる / pitch + でつま先が下がる（足首だけ回す方式の u と同じ向き）。
/// **両脚が足裏書きのときにだけ使える**（角度書きの脚は IK を通らないので回せない）。
inline void boardApply(FootPose foot[kNumSide], double roll, double pitch)
{
  if (roll == 0.0 && pitch == 0.0) {return;}
  const rk::Mat3 Q = rk::rotY(pitch) * rk::rotX(roll);
  const rk::Vec3 & pr = foot[kRight].p;
  const rk::Vec3 & pl = foot[kLeft].p;
  const rk::Vec3 c{0.5 * (pr.x + pl.x), 0.5 * (pr.y + pl.y), std::min(pr.z, pl.z)};
  for (int s = 0; s < kNumSide; ++s) {
    foot[s].p = c + Q * (foot[s].p - c);
    rpyFromMat(Q * matFromRpy(foot[s].rpy), foot[s].rpy);
  }
}
