...
// 1 行足すだけでこのクラスは追従する。
#ifndef ROBOONE_MOTION__SERVO_MAP_HPP_
#define ROBOONE_MOTION__SERVO_MAP_HPP_
...
/// サーボ ID -> 脚 6 軸の添字（Joint enum）。脚の軸でなければ -1。
///
/// モーション config が脚を角度で書くとき（``R_leg: {ID4: ...}``）の照合に使う。
/// 並びは kLegServoId のとおり足首が 6, 5 の順なので、ID から引くこと。
inline int legIndexOfServoId(int id)
{
  for (std::size_t j = 0; j < rk::kNumJoints; ++j) {
    if (kLegServoId[j] == id) {return static_cast<int>(j);}
  }
  return -1;
}
...
public:
  ...
  // --- 脚: T ポーズ基準角 [2] <-> 絶対サーボ角 [3] ---------------------
  // 人が config に書く・ティーチが吐くのは [2]、機体の中を流れるのは [3]。
  // その 2 つの間の定数（tpose_ref_）を外に出さずに済ませるための 2 本。

  /// T ポーズ基準角 [deg] -> 絶対サーボ角 [rad]。
  double leg_servo_from_tpose_deg(int side, std::size_t j, double deg) const
  {
    return tpose_ref_[side][j] + deg * M_PI / 180.0;
  }
  /// 絶対サーボ角 [rad] -> T ポーズ基準角 [deg]。
  double leg_tpose_deg_from_servo(int side, std::size_t j, double servo) const
  {
    return (servo - tpose_ref_[side][j]) * 180.0 / M_PI;
  }
...
#endif  // ROBOONE_MOTION__SERVO_MAP_HPP_
