...
def generate_launch_description():
    share = get_package_share_directory('roboone_motion')

    return LaunchDescription([
        # ★引数名に "config" を使わないこと。roboone_bringup から teleop と一緒に
        #   include されると、launch の設定値は兄弟の include にも漏れる。teleop 側も
        #   "config" を宣言しているので、同じ名前だと **teleop の yaml が motion に
        #   渡る**（2026-08-29 実機で motion が起動に失敗）。
        DeclareLaunchArgument(
            'motion_config',
            default_value=os.path.join(share, 'config', 'motion_node.yaml'),
