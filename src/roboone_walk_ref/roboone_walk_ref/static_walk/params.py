...
class StaticGaitParams:
    ...
    # 計画上の足間隔。**home_pose.yaml の foot.y の 2 倍と揃える。** motion ノードは
    # 計画の立位 (±W/2) をホーム姿勢の足へ平行移動するので、ずれた分 (W/2 - foot.y) だけ
    # 振り出し中の重心が支持足の中心から横へずれる。動歩行は計画の W を実機より広く
    # 取って横振りを増やしているが、静歩行で重心をずらすのは com_offset_y で行う。
    foot_spacing: float = 0.140     # [m]   左右の足間隔 W (2026-09-14 実機校正の 140mm)
    ...
    # 10mm で足間隔 140mm の移動に 1.47 s。大きくするほど ZMP の静的余裕が減る
    # (値ごとの表は static_gait.yaml)。
    zmp_tol: float = 0.010          # [m]
    ...
    # --- 遊脚 (SWING) -----------------------------------------------------
    t_swing: float = 0.6            # [s]   振り出しの時間 (重心は支持足の上で止めておく)
    ...
    # 実際の速さは 歩幅 / (重心移動 + 振り出し) で、動歩行の約 3 割
    # (全速前進で 1 歩 2.13 s・0.028 m/s)。
